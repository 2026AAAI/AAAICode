...
class Greedy():
    ...
    def deal_CL(self):
        ...
        for group in self.CL:
            assigned_sites = set()
            for j in group:
                sorted_i = sorted(self.X, key=lambda i: self.cij[i][j])
                for i in sorted_i:
                    if i not in assigned_sites:
                        self.assign_client[j] = i
                        assigned_sites.add(i)
                        break
...
def main(distance_matrix, Y, percent, i, facility_costs,file_name):
    ...
    all_cost = evaluate.cost_out(FL.P, FL.cij, facility_costs, FL.assign_client,
                               elapsed_time, f"output\\{file_name}\\{percent}%_Greedy_output_{i}.txt")
...
if __name__ == "__main__":
    ...
    for file_name in file_names:
        with open(f'{file_name}_Greedy_result.txt', "a") as file:
